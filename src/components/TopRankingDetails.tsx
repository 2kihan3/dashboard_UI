...
export default function TopRankingDetails<T>({ rows, title, note, preview = 5, children }: { rows: T[]; title: string; note: string; preview?: number; children: (items: T[], offset: number) => ReactNode }) {
  ...
  useEffect(() => { if (body.current) body.current.scrollTop = 0 }, [paged.current, size, open])
  return <>
    {children(rows.slice(0, count), 0)}
    <div className="os-top-more"><span>展示 {Math.min(count, paged.total)} / {paged.total} 条 · 最多 Top 100</span><button type="button" disabled={!paged.total} onClick={() => { setPage(1); setSize(10); setOpen(true) }}>查看更多<ArrowUpRight aria-hidden="true" /></button></div>
    <dialog ref={dialog} className="os-movement-dialog os-top-dialog" aria-labelledby={titleId} aria-describedby={noteId} onCancel={() => setOpen(false)} onClose={() => setOpen(false)} onClick={event => { if (event.target === event.currentTarget) { const rect = event.currentTarget.getBoundingClientRect(); if (event.clientX < rect.left || event.clientX > rect.right || event.clientY < rect.top || event.clientY > rect.bottom) setOpen(false) } }}>
      ...
      <div ref={body} className="os-top-dialog-body">{open && children(paged.rows, paged.offset)}</div>
