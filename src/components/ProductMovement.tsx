...
import { demoMovements, movementPage, rankMovements, type MovementKind, type MovementMetric, type ProductMovementRow } from '../data/productMovements'
...
function MovementTable({ rows, kind, offset = 0 }: { rows: ProductMovementRow[]; kind: MovementKind; offset?: number }) {
  return <div className="os-table-scroll"><table><thead><tr><th>排名 / 商品</th><th>支付金额 / 元</th><th>订单数（估算）</th><th>支付变化</th></tr></thead><tbody>{rows.map((row, index) => <tr key={row.id}><td><strong><span className="os-rank-number">{offset + index + 1}</span>{row.name}</strong><small>{row.sku} · {row.owner ?? '未归因'}</small></td><td>{money(row.payment)}</td><td>{row.estimatedOrders}</td><td><span className={kind === 'new' ? 'os-positive' : 'os-negative'}>{kind === 'new' ? '新增成交' : `${((row.payment / row.previousPayment - 1) * 100).toFixed(1)}%`}</span><small>前日 {money(row.previousPayment)} 元</small></td></tr>)}</tbody></table>{!rows.length && <p className="os-no-rows">没有匹配商品，调整上方筛选后再查看。</p>}</div>
}
export default function ProductMovement({ metric, search, owner, group }: { metric: MovementMetric; search: string; owner: string; group: string }) {
  const [preview, setPreview] = useState(5)
  const [kind, setKind] = useState<MovementKind | null>(null)
  const [page, setPage] = useState(1)
  const [size, setSize] = useState(10)
  const dialog = useRef<HTMLDialogElement>(null)
  const data = (key: MovementKind) => rankMovements(demoMovements[key].filter(row => (owner === '全部' || (owner === '未归因' ? !row.owner : row.owner === owner)) && (group === '全部' || row.group === group) && `${row.name} ${row.sku} ${row.id}`.toLowerCase().includes(search.trim().toLowerCase())), metric)
