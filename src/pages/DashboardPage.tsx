...
function GlobalDashboard({ period, platform, store, onPeriodChange, onPlatformChange, onStoreChange }: { period: Period; platform: PlatformName; store: string; onPeriodChange: (period: Period) => void; onPlatformChange: (platform: PlatformName) => void; onStoreChange: (store: string) => void }) {
  ...
  return (
    <>
      <section className={`data-scope-note global-dashboard-head ${dashboardTab === 'operations' ? 'global-dashboard-head--operations' : ''}`} data-prd-anchor="dashboard-global-scope">
        <div className="global-dashboard-title"><div><h2>全局经营看板</h2><span className="global-dashboard-title__notice">{dashboardTab === 'finance' ? '当前展示已接入的平台经营数据。' : '抖音运营看板 · 本地前端预览'}</span></div>{dashboardTab === 'finance' ? <span className="global-dashboard-title__context">{platform} · {periodText}</span> : null}</div>
