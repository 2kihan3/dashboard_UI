...
export function compareTraffic(days: TrafficDay[], date: string, metric: TrafficMetric) {
  ...
  return { yesterday: rate(days.filter(day => day.date === date), metric), previous: rate(days.filter(day => day.date === offset(1)), metric), seven: baseline(7), fourteen: baseline(14), sample: days.find(day => day.date === date) }
}
