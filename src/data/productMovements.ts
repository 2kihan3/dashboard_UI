...
export type MovementKind = 'up' | 'down'
export type MovementMetric = 'paymentChange' | 'ordersChange'
...
for (let index = 0; index < 100; index++) {
  ...
  demoMovements.up.push({ ...base, payment: payment * 1.25, estimatedOrders: orders + 8, previousPayment: payment, previousOrders: orders })
...
export function rankMovements(rows: ProductMovementRow[], metric: MovementMetric) {
  const rate = (row: ProductMovementRow) => metric === 'paymentChange' ? row.payment / row.previousPayment - 1 : row.estimatedOrders / row.previousOrders - 1
  return [...rows].sort((a, b) => Math.abs(rate(b)) - Math.abs(rate(a)) || a.id.localeCompare(b.id)).slice(0, 100)
}
