...
export function managementFeeFields(platform: PlatformName) {
  return Array.from(new Set(collectRows(platform, (row) => row.category === '管理费用' && !row.isTotal).map((item) => item.row.field)))
}
